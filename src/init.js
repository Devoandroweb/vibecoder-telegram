...
const TEMPLATE_BASE = path.join(__dirname, '..', 'templates', 'base');
const INSTALL_PACKAGES = ['node-telegram-bot-api', 'dotenv', 'chalk'];

function formatTargetPath(target) {
  return target ? path.resolve(process.cwd(), target) : process.cwd();
}
...
async function mergeScripts(targetPath, metas = [], logger) {
  ...
  await fs.writeJson(packageJsonPath, targetPackage, { spaces: 2 });
}

function installDependencies(targetPath, logger) {
  logger.info('Installing required dependencies in target project...');
  execSync(`npm install --save ${INSTALL_PACKAGES.join(' ')}`, {
    cwd: targetPath,
    stdio: 'inherit'
  });
...
async function initProject(target, options = {}) {
  ...
  try {
    ...
    if (useStarter) {
      const starterSource = path.join(TEMPLATE_BASE, 'starter');
      if (await fs.pathExists(starterSource)) {
        ...
        spinner.warn('Starter template not found, skipping starter copy.');
      }
    }
    ...
    const metas = [];
    const baseMetaPath = path.join(TEMPLATE_BASE, 'package.vibe.json');
    if (await fs.pathExists(baseMetaPath)) {
      metas.push(await fs.readJson(baseMetaPath));
    }
    if (useStarter) {
      const starterMetaPath = path.join(TEMPLATE_BASE, 'starter', 'package.vibe.json');
      if (await fs.pathExists(starterMetaPath)) {
        metas.unshift(await fs.readJson(starterMetaPath));
      }
    }

    await mergeScripts(targetPath, metas, {
    ...
    console.log('  3. npm run telegram:start');
  } catch (error) {
